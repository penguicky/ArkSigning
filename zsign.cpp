            ...
            uint32_t uZipLevel, int threadCount) 
{
    ...
    for (int i = 0; i < threadCount; i++) {
        workers.push_back(thread([&taskQueue, pSignAsset, bForce, bWeakInject, bDontEmbedProfile, 
                               arrDyLibFiles, strBundleId, strDisplayName, strBundleVersion,
                               uZipLevel, &completedTasks, &successfulTasks, &printMutex, &allTasks]() {
            SigningTask task;
    ...
    ZLog::PrintV("\n>>> Bulk signing completed: %d/%zu successful\n", 
              successfulTasks.load(), allTasks.size());
    
    return successfulTasks == allTasks.size();
}
