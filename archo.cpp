...
bool ZArchO::BuildCodeSignature(arksigningAsset *pSignAsset, bool bForce, const string &strBundleId, const string &strInfoPlistSHA1, const string &strInfoPlistSHA256, const string &strCodeResourcesSHA1, const string &strCodeResourcesSHA256, string &strOutput)
{
	...
	if (ZLog::IsDebug())
	{
		WriteFile("./.arksigning_debug/Requirements.slot.new", strRequirementsSlot);
		WriteFile("./.arksigning_debug/Entitlements.slot.new", strEntitlementsSlot);
		WriteFile("./.arksigning_debug/Entitlements.der.slot.new", strDerEntitlementsSlot);
		WriteFile("./.arksigning_debug/Entitlements.plist.new", strEntitlementsSlot.data() + 8, strEntitlementsSlot.size() - 8);
		WriteFile("./.arksigning_debug/CodeDirectory_SHA1.slot.new", strCodeDirectorySlot);
		WriteFile("./.arksigning_debug/CodeDirectory_SHA256.slot.new", strAltnateCodeDirectorySlot);
		WriteFile("./.arksigning_debug/CMSSignature.slot.new", strCMSSignatureSlot);
		WriteFile("./.arksigning_debug/CMSSignature.der.new", strCMSSignatureSlot.data() + 8, strCMSSignatureSlot.size() - 8);
		WriteFile("./.arksigning_debug/CodeSignature.blob.new", strOutput);
	}

	return true;
}

bool ZArchO::Sign(arksigningAsset *pSignAsset, bool bForce, const string &strBundleId, const string &strInfoPlistSHA1, const string &strInfoPlistSHA256, const string &strCodeResourcesData)
{
	if (NULL == m_pSignBase)
	{
