...
bool SlotParseRequirements(uint8_t *pSlotBase, CS_BlobIndex *pbi)
{
	...
	if (IsFileExists("/usr/bin/csreq"))
	{
		...
		while (nullptr != fgets(result, sizeof(result), cmd))
		{
	...
	if (ZLog::IsDebug())
	{
		WriteFile("./.arksigning_debug/Requirements.slot", (const char *)pSlotBase, uSlotLength);
	}
	return true;
}
...
bool SlotParseEntitlements(uint8_t *pSlotBase, CS_BlobIndex *pbi)
{
	...
	if (ZLog::IsDebug())
	{
		WriteFile("./.arksigning_debug/Entitlements.slot", (const char *)pSlotBase, uSlotLength);
		WriteFile("./.arksigning_debug/Entitlements.plist", (const char *)pSlotBase + 8, uSlotLength - 8);
	}
	return true;
}
...
bool SlotParseDerEntitlements(uint8_t *pSlotBase, CS_BlobIndex *pbi)
{
	...
	if (ZLog::IsDebug())
	{
		WriteFile("./.arksigning_debug/Entitlements.der.slot", (const char *)pSlotBase, uSlotLength);
	}
	return true;
}
...
bool SlotParseCodeDirectory(uint8_t *pSlotBase, CS_BlobIndex *pbi)
{
	...
	CS_CodeDirectory cdHeader = *reinterpret_cast<CS_CodeDirectory*>(pSlotBase);
	...
	if (ZLog::IsDebug())
	{
		...
		{
			WriteFile("./.arksigning_debug/CodeDirectory_SHA1.slot", (const char *)pSlotBase, uSlotLength);
		}
		else if (2 == cdHeader.hashType)
		{
			WriteFile("./.arksigning_debug/CodeDirectory_SHA256.slot", (const char *)pSlotBase, uSlotLength);
		}
	}
...
{
	strOutput.clear();
	if (nullptr == pCodeBase || uCodeLength <= 0 || strBundleId.empty() || strTeamId.empty())
	{
		return false;
	}
	...
	if (nullptr != pCodeSlotsData && (uCodeSlotsDataLength == uCodeSlots * cdHeader.hashSize))
	{ //use exists
...
bool SlotParseCMSSignature(uint8_t *pSlotBase, CS_BlobIndex *pbi)
{
	...
	if (ZLog::IsDebug())
	{
		WriteFile("./.arksigning_debug/CMSSignature.slot", (const char *)pSlotBase, uSlotLength);
		WriteFile("./.arksigning_debug/CMSSignature.der", (const char *)pSlotBase + 8, uSlotLength - 8);
	}
	return true;
}

bool SlotBuildCMSSignature(arksigningAsset *pSignAsset,
...
uint32_t GetCodeSignatureLength(uint8_t *pCSBase)
{
	...
	if (nullptr != psb && CSMAGIC_EMBEDDED_SIGNATURE == LE(psb->magic))
	{
...
bool ParseCodeSignature(uint8_t *pCSBase)
{
	...
	if (ZLog::IsDebug())
	{
		WriteFile("./.arksigning_debug/CodeSignature.blob", (const char *)pCSBase, LE(psb->length));
	}
	return true;
}
...
{
	pCodeSlots1Data = nullptr;
	pCodeSlots256Data = nullptr;
	...
	return ((nullptr != pCodeSlots1Data) && (nullptr != pCodeSlots256Data) && uCodeSlots1DataLength > 0 && uCodeSlots256DataLength > 0);
}
